...
type ComparePricesScreenRouteProp = RouteProp<
  MyRxStackParamList,
  "ComparePrices"
>;
...
export default function ComparePricesScreen() {
  ...
  const hasActiveFilters = useMemo(() => {
    return (
      inStockOnly || nearbyOnly || filterType !== "all" || minRating !== null
    );
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    if (nearbyOnly) {
      sources = sources.filter(
        (s) =>
          s.distance !== undefined && s.distance <= NEARBY_DISTANCE_THRESHOLD,
      );
    }
    ...
    if (filterType === "generic") {
      ...
      sources = sources.filter(
        (s) => s.isGenericOffer === false || s.isGenericOffer === undefined,
      );
    }

    if (minRating !== null) {
      sources = sources.filter(
        (s) => s.patientRating !== undefined && s.patientRating >= minRating,
      );
    }
  ...
  const filteredLowestPriceSource = useMemo(() => {
    ...
    return filteredAndSortedSources.find(
      (s) => s.price === filteredLowestPrice,
    );
  ...
  useEffect(() => {
    if (
      selectedPharmacy &&
      !filteredAndSortedSources.some((s) => s.id === selectedPharmacy)
    ) {
  ...
  const renderPriceCard = (source: PriceSource, isFilteredLowest: boolean) => {
    return (
      <Pressable key={source.id} onPress={() => setSelectedPharmacy(source.id)}>
        <View
          style={[
            styles.priceCard,
        ...
        >
          {isFilteredLowest && (
            <View
              style={[styles.lowestBadge, { backgroundColor: theme.primary }]}
            >
              <Feather name="award" size={14} color={theme.buttonText} />
              <ThemedText
                style={[styles.lowestBadgeText, { color: theme.buttonText }]}
              >
                Lowest Price
              </ThemedText>
            </View>
          ...
          {selectedPharmacy === source.id && (
            ...
            >
              ...
              <ThemedText
                style={[styles.selectedText, { color: theme.primary }]}
              >
                Selected
              </ThemedText>
            </View>
  ...
  return (
    <ScreenScrollView>
      <View style={styles.header}>
        ...
        <ThemedText style={[styles.supply, { color: theme.textSecondary }]}>
          {priceData.supply}
        </ThemedText>
      </View>
      ...
      <View style={[styles.summaryCard, { backgroundColor: theme.card }]}>
        {filteredAndSortedSources.length === 0 ? (
          <View style={styles.emptySummaryContainer}>
            ...
            <ThemedText
              style={[styles.emptySummaryText, { color: theme.textSecondary }]}
            >
          ...
          <View style={styles.summaryRow}>
            <View>
              ...
              >
                {hasActiveFilters && filteredLowestPrice !== null
                  ? "Lowest (Filtered)"
                  : "Lowest Price"}
              </ThemedText>
            ...
            <View>
              <ThemedText
                style={[styles.summaryLabel, { color: theme.textSecondary }]}
              >
                {hasActiveFilters && filteredAveragePrice !== null
                  ? "Average (Filtered)"
                  : "Average Price"}
              </ThemedText>
              <ThemedText style={styles.summaryValue}>
                $
                {(filteredAveragePrice !== null
                  ? filteredAveragePrice
                  : priceData.averagePrice
                ).toFixed(2)}
              </ThemedText>
              {filteredLowestPrice !== null && filteredAveragePrice !== null ? (
                <ThemedText
                  style={[styles.summarySavings, { color: theme.success }]}
                >
                  Save $
                  {(filteredAveragePrice - filteredLowestPrice).toFixed(2)}
                </ThemedText>
              ) : (
                <ThemedText
                  style={[styles.summarySavings, { color: theme.success }]}
                >
                  Save $
                  {(priceData.averagePrice - priceData.lowestPrice).toFixed(2)}
                </ThemedText>
              )}
            </View>
          </View>
      ...
      <View style={styles.filterSection}>
        <View style={styles.filterHeader}>
          ...
          {hasActiveFilters && (
            ...
            >
              <ThemedText
                style={[styles.clearAllText, { color: theme.primary }]}
              >
                Clear All
              </ThemedText>
            </Pressable>
      ...
      <View style={styles.priceList}>
        <ThemedText style={styles.listTitle}>
          ...
          {filteredAndSortedSources.length === 1 ? "Pharmacy" : "Pharmacies"}{" "}
          Found
        </ThemedText>
        {filteredAndSortedSources.length === 0 ? (
          <View style={styles.emptyFilterContainer}>
            ...
            <ThemedText
              style={[styles.emptyFilterText, { color: theme.textSecondary }]}
            >
            ...
            >
              <ThemedText
                style={[styles.clearFiltersText, { color: theme.buttonText }]}
              >
                Clear Filters
              </ThemedText>
            </Pressable>
          ...
          <React.Fragment>
            {filteredAndSortedSources.map((source) => {
              const isLowestInFiltered = filteredLowestPriceSource
                ? source.id === filteredLowestPriceSource.id
                : false;
